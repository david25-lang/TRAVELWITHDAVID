    ...
    <div className="pb-24">
      ...
      <section className="shell mt-12 space-y-8">
        <div className="flex items-center justify-between gap-4">
          <p className="text-sm text-slate-400">{fullItems.length} saved trip{fullItems.length === 1 ? '' : 's'}</p>
        ...
        {fullItems.length ? (
            ...
            {fullItems.map((item) => (
              <article key={`${item.type}-${item.slug}`} className="surface overflow-hidden rounded-[1.75rem]">
                ...
                <div className="p-5">
                  ...
                  <p className="mt-2 text-sm text-slate-400">{item.meta || item.source?.country || item.source?.location}</p>
                  <p className="mt-4 text-sm text-slate-300">{formatCurrency(item.price || item.source?.price || item.source?.startingPrice)}</p>
                  <div className="mt-5 flex items-center justify-between gap-4">
                    <Link to={item.type === 'tour' ? `/tours/${item.slug}` : `/destinations/${item.slug}`} className="text-amber-200">View</Link>
                    <button type="button" className="text-sm text-slate-400" onClick={() => removeItem(item.slug, item.type)}>Remove</button>
                  </div>
                </div>
              </article>
            ))}
          ...
          <div className="surface rounded-[2rem] p-12 text-center text-slate-300">
